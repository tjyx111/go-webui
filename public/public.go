...
func EleClickByCssSelector(wd selenium.WebDriver, args ...string) error {
	if len(args) != 1 {
		return fmt.Errorf("invalid args, expect args[0] xpath")
	}
	return ClickBy(wd, selenium.ByCSSSelector, args[0])
}
...
func IsDisplayedWithTextByXpath(wd selenium.WebDriver, args ...string) error {
	if len(args) != 2 {
		return fmt.Errorf("invalid args, expect args[0] xpath, args[1] expect")
	}
	return IsDisplayedWithText(wd, selenium.ByXPATH, args[0], args[1])
}

func IsDisplayedWithText(wd selenium.WebDriver, by, key, expect string) error {
	ele, err := wd.FindElement(by, key)
	if err != nil {
		return err
	}
	...
	if text != expect {
		return fmt.Errorf("element text is not %s", expect)
	}
	return nil
}
